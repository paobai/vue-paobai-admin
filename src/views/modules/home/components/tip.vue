  ...
  <a-card :loading="loading" class="tip-card" title="TIP/技术栈">
    <a-typography style="margin-top: -20px">
      ...
      <a-typography-paragraph
        ...
        比如vben应该是最广为人知的，但是我觉得里面的东西有点多了。太多了会影响开发体验。
        比如vue-il8n国际化，这个对项目耦合度太高了。我相信开发给国内用户的人来说不需要国际化。
        加之很多框架对用户权限菜单支持不是特别好。以及目前没有（可能我不知道）基于Arco
      ...
      <a-typography-paragraph>
        <ul>
          ...
          <li>数据模拟使用<a-link href="http://mockjs.com/">Mockjs</a-link>，并搭配vite-plugin-mock插件食用</li>
        </ul></a-typography-paragraph
      >
      <a-typography-title :heading="5">Tip</a-typography-title>
      <a-typography-paragraph>
        <ul>
          <li>项目部分页面copy自<a-link href="http://vue-pro.arco.design/login">Arco Design Pro</a-link></li>
          <li>
            之所以不用Arco Design Pro是因为有几点原因：
            <ul>
              <li>
                路由权限实现太过单调，修改角色所对应显示的菜单需要修改前端代码而不能通过修改数据来实现。该项目可通过配置角色获取能访问的菜单通过接口返回后动态载入，可以在数据层面实现配置。
              </li>
              <li>
                菜单显示位置不够多态。只能在侧边栏显示菜单。本人的项目很多一级菜单在导航栏（顶部），二级菜单在侧边栏，或者都有可能。
                该项目有三种菜单显示方式：
                <ul>
                  <li>菜单都在侧边栏</li>
                  <li>菜单都在侧边栏导航栏（顶部）</li>
                  <li>
                    一级菜单在导航栏，选中的子菜单在侧边栏显示，同时选中一级菜单若没有子菜单则自动隐藏侧边栏（该菜单方式可以实现首页全屏的应用场景）。
                  </li>
                </ul>
              </li>
              <li></li>
